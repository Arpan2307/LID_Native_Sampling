...
def load_and_sample_datasets(base_path, config, sample_size_en, sample_size_hi, train_sample_size):
    ...
    if config in [1, 3] and sample_size_en > 0:
        ...
        extra_tokens += [eng_tokens[i] for i in indices]
        extra_labels += [eng_labels[i] for i in indices]
    ...
    if config in [2, 3] and sample_size_hi > 0:
        ...
        extra_tokens += [hin_tokens[i] for i in indices]
        extra_labels += [hin_labels[i] for i in indices]

    combined = list(zip(base_tokens + extra_tokens, base_labels + extra_labels))
    random.shuffle(combined)

    if 0 < train_sample_size < len(combined):
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), f"best_model_config_{args.train_config}_{args.model}.pt")
        else:
            patience_counter += 1
    ...
    model.load_state_dict(torch.load(f"best_model_config_{args.train_config}_{args.model}.pt"))
    ...
    os.remove(f"best_model_config_{args.train_config}_{args.model}.pt")
